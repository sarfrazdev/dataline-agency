...
//       path: 'items.product',
//       select: 'name images prices brand stock', // include all required fields
//     });
...
//       // Get fallback price based on role
//       const fallbackPrice = product?.prices?.[userRole] || 0;

//       // Get dynamic price using quantity slabs
//       const quantityBasedPrices = product?.prices?.quantityBased?.[userRole] || [];

//       const dynamicPrice = getDynamicPrice(quantity, quantityBasedPrices, fallbackPrice);
...
//     const cart = await Cart.findOne({ user: req.user._id }).populate({
//       path: 'items.product',
//       select: 'name image images price prices brand stock quantityBasedPrices',
//     });
...
//       // Determine base role price
//       const fallbackPrice = product?.prices?.[userRole] || 0;

//       // Get slab price if available
//       const finalPrice = getDynamicPrice(quantity, product?.quantityBasedPrices, fallbackPrice);
