  ...
  const fetchOrders = async () => {
    try {
      const { data } = await axiosInstance.get(
        `${BASE_URL}${API_PATH.DISTRIBUTOR_ADMIN.GET_ORDERS}`,
        {
          headers: {
...
const handleStatusChange = async (orderId, newStatus) => {
  try {
    await axiosInstance.put(
      `${BASE_URL}${API_PATH.DISTRIBUTOR_ADMIN.UPDATE_ORDER_STATUS(orderId)}`,
      { status: newStatus }, 
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
          ...
          <div className="space-y-6">
            {orders.map((order) => (
              ...
              >
                ...
                <div>
                  <h3 className="text-sm font-bold mb-1 text-gray-800">👤 Customer Info</h3>
