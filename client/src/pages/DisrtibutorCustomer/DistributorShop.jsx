...
//   'Printer Adjustment Software', 'CP Plus', 'Hikvision', 'Sony', 'Nikon', 'Zebronics','TP-Link', 'D-Link','Consistent','HikVision',
...
//  // ✅ Read brand from URL and set selectedBrand
//   useEffect(() => {
...
//     try {
//       const response = await axiosInstance.get(`${BASE_URL}${API_PATH.PRODUCTS.GET_ALL}`);
...
//       if (!token) return;
//       const response = await axios.get(`${BASE_URL}${API_PATH.WISHLIST.GET}`, {
//         headers: { Authorization: `Bearer ${token}` }
//       });
//       const wishlistItems = response.data?.wishlist || [];
//       const wishlistIds = wishlistItems.map(item => item?.product?._id).filter(Boolean);
...
//                 setCurrentPage(1);
//               }}
//               className="w-full md:w-1/4 px-4 py-2 rounded border bg-gray-100 text-black"
//             >
...
//               className="w-full md:w-1/2 px-4 py-2 rounded border bg-gray-100 text-black"
//             />
//           </div>
...
//               <div key={product._id} className="bg-white text-black rounded shadow hover:shadow-lg p-4 flex flex-col">
...
//                   <span className="font-bold text-blue-600">₹{product?.prices?.[role] || 'N/A'}</span>
...
//                       <Heart
//                         className={`w-4 h-4 ${
//                           wishlist.includes(product._id) ? 'fill-red-500 text-red-500' : 'text-red-500'
//                         }`}
//                       />
//                     </button>
//                     {product.stock === 0 ? (
